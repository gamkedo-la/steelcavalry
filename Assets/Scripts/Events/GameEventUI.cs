...
public class GameEventUI : ScriptableObject
{
	...
	public void Raise( UIEvent parameter, float percentFull = 1.0f )
	{
		// Back to front so we can unsubscribe from within the loop and still be ok
		for ( int i = listeners.Count - 1; i >= 0; i-- )
		{
			listeners[i].OnEventRaised( parameter, percentFull ); // Notifies GameEventListenerFloat that this event has been fired
		}
	}
